...
% readers of the corresponding journal paper, "Closing yield gaps is crucial 
% to avoid large-scale greenhouse gas emissions from agricultural expansion 
% for maize", to allow transparency and reproducibility. The authors have  
% no additional resources to provide free training, education, consultation, 
% or other support in connection with the codes and the data. However, the 
% authors welcome any joint development or collaboration opportunities, for 
% which readers can contact the corresponding author of the paper.
...
% 3.6 Costs: Expansion
 ...
 %% 4. Prepare Variables for Loop
    ...
    % Load FAO data for calibration factor generation
        FAOext_production = csvread('FAOext_production.csv'); %[country code, tonne]
        FAOint_production = csvread('FAOint_production.csv'); 
    % Create empty calibration factor tables
        extense_calibra_by_country = zeros(size(FAOext_production));
        extense_calibra_by_country (:,1)= FAOext_production (:,1); 
        intense_calibra_by_country = zeros(size(FAOint_production));
        intense_calibra_by_country (:,1)= FAOint_production (:,1); 
        harvested_area_correction_factor_int = zeros(size(FAOint_production)); 
        harvested_area_correction_factor_int(:,1)= FAOint_production (:,1); 
        harvested_area_calibration_factor = zeros(size(harvested_area_correction_factor_int));
        harvested_area_calibration_factor(:,1) = harvested_area_correction_factor_int(:,1); 
...
% 5.4 Format Costs and Constraints for Optimization
 ...
 %% 6. Generate Calibration Factors per Country 
        % Requires optimization_files_"f"precal.mat
   
        load('country_map_3.mat');

        if (g == 1) & (f < 7)

        rownum_ext = numel(extense_constr_db(:,1)); 
        extense_calibr1 = zeros(rownum_ext,4); 
        extense_calibr1(:,1) = extense_constr_db(:,1); 
        extense_calibr1(:,2) = country_map_3(extense_calibr1(:,1)); 
        extense_calibr1(:,3) = extense_constr_db(:, 4); % [t]
        extense_calibr1(:, 4) = extense_cost_db_0(:, 4); % [$/t]

        rownum_int = numel(intense_constr_db(:,1)); 
        intense_calibr1 = zeros(rownum_int,4); 
        intense_calibr1(:,1) = intense_constr_db(:,1); 
        intense_calibr1(:,2) = country_map_3(intense_calibr1(:,1)); 
        intense_calibr1(:,3) = intense_constr_db(:, 4); % [t]
        intense_calibr1(:,4) = intense_cost_db(:, 4); % [$/t]

        sort_costs_ext = sortrows(extense_calibr1,[2 4]); 
        sort_costs_int = sortrows(intense_calibr1,[2 4]); 
        
        extense_shortfall = [];
        shortfall_counter_ext = 1;

        for i = 1:size(extense_calibra_by_country,1)
          country = extense_calibra_by_country(i,1); 
          temp_matrix1 = sort_costs_ext(sort_costs_ext(:,2) == country, :); 
          cumul_prod = 0; 
          FAOext_production_country = FAOext_production(FAOext_production (:,1) == country, f+1); 
          prod_country = temp_matrix1(:,3); 
          price_country = temp_matrix1(:,4);

          min_price = 1.65;
          max_price = 15000;

          if FAOext_production_country <= 0 
                if isempty(price_country(price_country>0))== 1
                    min_price = 1.65;
                else
                    min_price = min(price_country(price_country>0)); 
                end
                extense_calibra_by_country(i, f+1) = 165- min_price; 

          else
            for j = 1:size(temp_matrix1,1) 
                cumul_prod = cumul_prod + temp_matrix1(j, 3); 
                if cumul_prod >= FAOext_production_country 
                    max_price = temp_matrix1(j, 4); 
                break
                elseif j == size(temp_matrix1, 1) 
                    max_price = temp_matrix1(j, 4); 
                    extense_shortfall(shortfall_counter_ext, 1) = country; 
                    extense_shortfall(shortfall_counter_ext, 2) = FAOext_production_country - cumul_prod ;
                    shortfall_counter_ext = shortfall_counter_ext +  1;
                end

            end
            extense_calibra_by_country(i, f+1) = 150- max_price; 
         end
        end


        intense_shortfall = [];
        shortfall_counter_int = 1 ;

        for i = 1:size(intense_calibra_by_country,1)
          country = intense_calibra_by_country(i,1); 
          temp_matrix2 = sort_costs_int(sort_costs_int(:,2) == country, :); % [t]
          cumul_prod = 0; 
          FAOint_production_country = FAOint_production(FAOint_production (:,1) == country, f+1); 
          prod_country = temp_matrix2(:,3); 
          price_country = temp_matrix2(:,4);

          min_price = 1.65;
          max_price = 15000;

          if FAOint_production_country <= 0 
                if isempty(price_country(price_country>0)) == 1
                    min_price = 1.65;
                ...
                intense_calibra_by_country(i, f+1) = 165- min_price; 
          else
            for j = 1:size(temp_matrix2,1) 
                cumul_prod = cumul_prod + temp_matrix2(j, 3); % [t]
                if cumul_prod >= FAOint_production_country 
                    max_price = temp_matrix2(j, 4); 
                    break
                elseif j == size(temp_matrix2, 1) 
                    max_price = temp_matrix2(j, 4); % [$]
                    intense_shortfall(shortfall_counter_int, 1) = country; 
                    intense_shortfall(shortfall_counter_int, 2) = FAOint_production_country - cumul_prod ;
                    harvested_area_correction_factor_int(harvested_area_correction_factor_int(:,1) == country, f+1)= FAOint_production_country/cumul_prod;
                    shortfall_counter_int = shortfall_counter_int +  1 ;

                end            
             end
          intense_calibra_by_country(i, f+1) = 150- max_price; 
          end
        end

        outputfile_counter = csvread('outputfile_counter.csv');
        save extense_calibra_by_country.mat extense_calibra_by_country 
        save intense_calibra_by_country.mat intense_calibra_by_country 
        save harvested_area_correction_factor_int.mat harvested_area_correction_factor_int 
        csvwrite(['extense_calibra_', num2str(outputfile_counter), '_year', num2str(f+2009), '_yield_sel_', num2str(g), '.csv'],extense_calibra_by_country); 
        csvwrite(['intense_calibra_', num2str(outputfile_counter), '_year', num2str(f+2009), '_yield_sel_', num2str(g), '.csv'],intense_calibra_by_country); 
        csvwrite(['harv_area_corr_f_int_', num2str(outputfile_counter), '_year', num2str(f+2009), '_yield_sel_', num2str(g), '.csv'], harvested_area_correction_factor_int);
        csvwrite('extense_calibra_by_country_calibrated.csv',extense_calibra_by_country);
        csvwrite('intense_calibra_by_country_calibrated.csv',intense_calibra_by_country);
        csvwrite('harvested_area_correction_factor_int.csv', harvested_area_correction_factor_int);


        for p = 1:size(harvested_area_correction_factor_int(:,1))

            if harvested_area_correction_factor_int(p,f+1) == 0
                    harvested_area_calibration_factor(p,f+1) = 1;
            elseif harvested_area_correction_factor_int(p,f+1) >100
                    harvested_area_calibration_factor(p,f+1) = 1;
            else harvested_area_calibration_factor(p,f+1) = harvested_area_correction_factor_int(p,f+1);
            end
        end

        save harvested_area_calibr_factor.mat harvested_area_calibration_factor;
        csvwrite('harvested_area_calibration_factors.csv',harvested_area_calibration_factor);

        end


 %% 7. Generate GLUC Year "f" Calibrated Results
        % Generates optimization_files_"f".mat 

 % 7.1 Expansion Calibration 
  ...
  % 7.2 Yield Improvement Recalculation
...
% 7.3 Intensification marginal production recalculation
...
% 7.4 Intensification Calibration 
...
% 7.5 Format Costs and Constraints for Optimization
 ...
 %% 13. Prepare Next Loop Run
     ...
     % Update the yield matrix 
       ...
       % Add step to counter
        ...
        clearvars -except f g existing_cropland_harvest_area_ha yield_pot N_app_rate_ext N_app_rate P_app_rate K_app_rate extense_cost_0 extense_results_raster_ha cumsum_extense_results_raster_ha area annual_yield_increase_perc_final yield2010 b_nut_geo c_N_geo c_P2O5_geo b_K2O_geo c_K2O_geo data_country_codes extense_calibra_by_country intense_calibra_by_country harvested_area_correction_factor_int harvested_area_calibration_factor FAOext_production FAOint_production yield_pot_max yield_selection difference_max_pot_vs_2010_yield existing_cropland_harvest_area_ha_year0 max_cap_cost
